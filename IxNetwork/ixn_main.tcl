...
proc ::IxNetwork::GetProtocols {} {
	variable ::TrafficGenerator::objectTypes_Array
	set protocols ListDelimiter\n
	foreach type {ProtocolEndpoint Protocol} {\
		if {[lsearch [array names objectTypes_Array] $type] >= 0} {
			foreach protocol [::TrafficGenerator::GetObjectTypes $type] {
				append protocols $protocol\t
			}	
		}
	}
	append protocols ListDelimiter\n
	if {[lsearch [array names objectTypes_Array] ProtocolNgpf] >= 0} {
		foreach protocol [::TrafficGenerator::GetObjectTypes ProtocolNgpf] {
			append protocols $protocol\t
		}
	}
	return $protocols
